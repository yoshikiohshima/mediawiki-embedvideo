...
class VideoService {
	...
	static private $services = [
		...
		'youtubeIA' => [
			'embed'			=> '<script type="text/javascript">var interval;var player;console.log("%1$s");function onPlayerStateChange(e) {if (e.data == YT.PlayerState.PLAYING) {interval = window.setInterval(updateEventHighlight, 500); /* update every half-second */ updateEventHighlight(); }; if (e.data == YT.PlayerState.ENDED || e.data == YT.PlayerState.PAUSED || e.data == YT.PlayerState.BUFFERING) { window.clearInterval(interval);}};function onYouTubeIframeAPIReady() {player = new YT.Player("ia-video", {height: %3$d, width:  %2$d, videoId: "%1$s", events: { "onStateChange": onPlayerStateChange }, playerVars: {rel: 0, showinfo: 0},});}</script><div id="ia-holder"><div id="ia-video"></div><div id="ia-caption"></div></div><script type="text/javascript">function iaLoad(){console.log("i" + Date.now())};iaLoad();</script>',
			'default_width'	=> 640,
			'default_ratio'	=> 1.77777777777778, // (16 / 9)
			'https_enabled'	=> true,
			'url_regex'		=> [
